...
Example, prompts given directly on the command line:
    uv run python scripts/generate_audio_stable_audio.py \
...
Example, prompts read from a CSV file with a `prompt` column:
    uv run python scripts/generate_audio_stable_audio.py --dataset datasets/trumpet_prompts_dataset.csv \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate audio with Stable Audio Open for a list of prompts.",
    ...
    generation.add_argument("--guidance-scale", type=float, default=7.0, help="classifier free guidance scale")
        ...
        for batch_index, start in enumerate(range(0, len(prompts), args.batch_size)):
            ...
            with torch.inference_mode():
                output = pipe(
                    ...
                    audio_end_in_s=args.audio_length_in_s,  # `--audio-length-in-s` maps to the clip's end offset
                    num_inference_steps=args.num_inference_steps,
                    guidance_scale=args.guidance_scale,
                    generator=generators,
                    output_type="np",
                )
            waveforms = output.audios  # (batch, channels, samples)
