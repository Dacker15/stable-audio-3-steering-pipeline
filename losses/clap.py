...
class ClapLoss(nn.Module):
    ...
    returns `1 - cosine_similarity(audio_embeds, text_embeds)`, so that minimizing the loss pulls the
    generated audio towards the text.
    ...
    def from_pretrained(cls, model_id: str = DEFAULT_CLAP_MODEL_ID, reduction: str = "mean") -> "ClapLoss":
        ...
        return cls(
            text_encoder=ClapModel.from_pretrained(model_id),
    ...
    def embed_dim(self) -> int:
        r"""Dimension of the shared audio-text space, i.e. what `encode_text` and `encode_audio` return."""
